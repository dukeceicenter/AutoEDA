...
The MCP EDA (Model Context Protocol for Electronic Design Automation) framework provides a unified interface for automating digital design implementation flows using natural language processing and RESTful APIs. The system integrates multiple EDA tools (Synopsys Design Compiler and Cadence Innovus) through independent microservices, enabling intelligent automation of the complete RTL-to-GDSII design flow.

### Architecture

The framework consists of:
- **MCP Agent Client** (Port 8000): Main interface that processes natural language queries and routes to appropriate services
...
The main endpoint for natural language EDA operations. This endpoint processes user queries and automatically selects the appropriate MCP server.
...
If you prefer, you may invoke each `/.../run` endpoint directly – the JSON snippets below are exactly what the servers need.
...
```json
{
  ...
  "floorplan_path": "/home/yl996/proj/mcp-eda-example/designs/des/FreePDK45/implementation/floorplan.enc.dat",
  "syn_ver": "cpV1_clkP1_drcV1",
...
```json
{
  ...
  "powerplan_path": "/home/yl996/proj/mcp-eda-example/designs/des/FreePDK45/implementation/powerplan.enc.dat"
}
```
...
```json
{
  ...
  "placement_path": "/home/yl996/proj/mcp-eda-example/designs/des/FreePDK45/implementation/placement.enc.dat"
}
```
...
```json
{
  ...
  "cts_path": "/home/yl996/proj/mcp-eda-example/designs/des/FreePDK45/implementation/cts.enc.dat"
}
```
...
```json
{
  ...
  "route_path": "/home/yl996/proj/mcp-eda-example/designs/des/FreePDK45/implementation/route_opt.enc.dat"
}
```
...
### Example 1: Complete Design Flow for DES Design
...
# 2. Synthesis Compile
curl -X POST http://localhost:13334/compile/run \
  -H "Content-Type: application/json" \
  -d '{
    "design": "des",
    "tech": "FreePDK45",
...
# 4. Power Planning
curl -X POST http://localhost:13336/power/run \
  -H "Content-Type: application/json" \
  -d '{
    "design": "des",
    "top_module": "des3",
...
curl -X POST http://localhost:13338/cts/run \
  -H "Content-Type: application/json" \
  -d '{
    "design": "des",
    "top_module": "des3",
...
# 7. Routing
curl -X POST http://localhost:13339/route/run \
  -H "Content-Type: application/json" \
  -d '{
    "design": "des",
    "top_module": "des3",
...
# 8. Save Design
curl -X POST http://localhost:13440/save/run \
  -H "Content-Type: application/json" \
  -d '{
    "design": "des",
    "top_module": "des3",
...
### Example 2: VHDL Design (B14) with Different Clock Configuration

```bash
# For VHDL designs, ensure config.tcl has correct clock name
# config.tcl for b14:
# set CLOCK_NAME "clock"  # Must match VHDL port name

# Synthesis Setup for B14
curl -X POST http://localhost:13333/setup/run \
  -H "Content-Type: application/json" \
  -d '{
    "design": "b14",
...
# Synthesis Compile with different parameters
curl -X POST http://localhost:13334/compile/run \
  -H "Content-Type: application/json" \
  -d '{
    "design": "b14",
    "tech": "FreePDK45",
    "version_idx": 1,  # Different configuration
    "force": true
  }'
```

### Example 3: Using Natural Language Interface

```bash
# All the above operations can be performed using natural language
